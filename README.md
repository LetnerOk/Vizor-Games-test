...
Но способ оценки LTV через кумулятивный ARPU хорош тем, что можно экстраполировать тренд и построить прогноз LTV.
...
Если снизился CPI, но упал Retention_1, важно понять, мы закупили менее заинтересованную и неподходящую аудиторию или просто изменилось ее поведение.  
В таком случае важно смотреть следующие метрики:  
1. Конверсию в платящего юзера
2. ARPU
3. LTV
4. Более долгосрочный Retention (3, 7,..).

Если снижается конверсия в платящего, падает ARPU и LTV, значит закупаем нецелевую аудиторию и мы сливаем бюджет в ведро. Стоит пересмотреть кампанию.
Если Retention восстанавливается, игроки возвращаются позже, смотрим на показатели монетизации. Если повышается конверсия в платящего и ARPU не падает, юзеры начинают, например, платить позже, то кампания может быть успешной.
...
3. Cредняя длительность сессии в зависимости от порядкового номера сессии. Можно сразу исключить выброс в CTE  
   (WHERE user_id != 117411339), чтобы избежать аномальной длительности в сессии 199.   
...
Большую часть revenue здесь сгенерировал юзер 47862140 с двумя крупными платежами с разницей в 20 сек на общую сумму 15.2.
...
2. Отслеживание открытия банка даст нам знание и уверенность, что у игрока в очередном релизе открывается основной источник нашего дохода. Кроме того, если многие игроки заходят в банк, но не совершают платеж, нужно поработать над продуктами в банке.
